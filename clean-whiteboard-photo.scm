...
(define (cwb-get-top-layer arg)
  (let* (
         (layer-info (gimp-image-get-layers arg))
         (num-layers (car layer-info))
         (layer-ids (cadr layer-info)))
    (aref layer-ids 0)))

(define (clean-whiteboard-photo inFile)
  (let* (
         (theImage (car (gimp-file-load RUN-NONINTERACTIVE inFile inFile)))
         (mainLayer (cwb-get-top-layer theImage))
    ...
    (gimp-file-save RUN-NONINTERACTIVE theImage (car (gimp-image-get-active-layer theImage)) inFile inFile)
